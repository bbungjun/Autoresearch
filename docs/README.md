...
- [Plan — 로컬 Research Harness MVP](plans/2026-08-15-local-research-harness-mvp.md) — 현행 executor와 분리된 로컬 경로에서 사람이 준 가설·ExperimentCard부터 Sealed Judge·반복 실행·ledger·REPORT까지의 구현 순서 (#769). Stage B snapshot builder와 Stage C 입력 foundation은 구현했지만 fixture·Judge handoff orchestration이 대기 중이므로 이 active plan은 archive하지 않는다.
- [Spec — Research Harness P0-1 평가 snapshot](specs/2026-08-31-research-harness-evaluation-snapshot.md) — P0-1 구현 정본: `slate_id`, 평가 기간·click 귀속, label 봉인, split·fingerprint (#17). Stage B와 Stage C 입력 foundation 이후 orchestration이 남아 있어 살아있는 계약으로 유지한다.
...
- [Research Harness P0-1 평가 snapshot](specs/2026-08-31-research-harness-evaluation-snapshot.md) —
  ...
  content-addressed write-once snapshot 계약 (#17, Stage C 입력 foundation 이후 orchestration 대기)
- [로컬 Research Harness MVP 구현 계획](plans/2026-08-15-local-research-harness-mvp.md) —
  ...
  (#769, Stage C fixture·Judge handoff orchestration 대기라 archive하지 않음)
