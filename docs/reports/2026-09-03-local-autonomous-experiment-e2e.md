...
## 27. 날짜순 선행 이력과 학습 날짜를 분리하기 — #105

최근 행동이 모두 0이던 원인을 해결하기 위해 기존 v1을 보존하는 32일 history fixture v2를
추가했다. 8/2~8/31은 warm-up, 9/1은 학습, 9/2는 학습 label scan tail,
9/3은 평가, 9/4는 평가 scan tail이다. 모든 날짜는 KST 기준이다.
Warm-up impression 144,000행을 fit 대상에서 제외하고 9/1의 4,800행만 선택했다.
학습 당일 이후 이벤트 10,800행을 제거해도 과거 피처가 동일함을 실제 6개 split에서 확인했다.

15피처 대 최근 행동 제거 10피처를 36회 비교했다. 최근 시청 시간은 실제 값으로
조립됐고 포함 모델 18개 모두 분기에 사용했다. 다만 NDCG@10은 validation 0.934282,
final 0.945335로 각 paired 관측이 동률이고 Recall도 모두 1.0이라 사전 판정은
`not_supported`다. 확률 오차의 일부 평균 개선만 관측했다. 실행은 779.014초였고,
새 final 3개만 한 번씩 소비했다. 기존 #16/#103 결과와 final은 재사용하지 않았다.

시간 분리 기반은 유지하고 production 계약은 바꾸지 않는다. 다음은 행동 빈도·관심 변화가
다양한 평가 환경에서의 검증이다. 상세 근거·분산·비용·한계는
[실측 보고서](2026-09-05-temporal-warmup-experiment.md)에 보존한다.

## 28. 사용자별 활동과 관심 변화가 있는 raw 이력 생성 — #107

#105의 count 상수를 해소하기 위해 기존 fixture를 변경하지 않는 별도 합성 생성기를
추가했다. 8/3~9/1의 30일 선행 이력, 9/2 학습일, 9/3 귀속 확인일을 순차 생성했다.
접속 여부·8/16/24개 노출·클릭 성향·시청 시간과 일부 사용자 관심 전환을 seed로 고정하고,
production event 확장/Parquet writer를 재사용했다. 미래 선호는 학습 metadata에서 분리했다.

3개 seed에서 168,548개 이벤트를 53.970초에 생성·감사했다. 전체 사용자와 학습일 실제
노출 모두 최근 5개 피처에 분산이 생겼고, 전환 전후 클릭 관측 사용자 95/88/92명에서
새 선호 카테고리 비율 증가가 확인됐다. 독립 리뷰가 raw 로그에서 통계와 시간 순서·전역
event ID 고유성을 직접 대조했다. 관련 79개 회귀, 추가 보완 후 신규 14개 회귀가 통과했다
(중복 합산하지 않음). 같은 seed 재생성은 bytes가 같으며 출력 덮어쓰기를 거절한다.

이는 데이터 품질 검증이다. 학습·ranking 비교·final 소비는 없었고, 선호 카테고리를
강하게 우선하는 합성 규칙의 현실성은 미입증이다. 후속 비교는 개발 데이터에서 학습 입력을
조립하고 신규 평가 조건을 사전 등록하는 순서로 진행한다. 전체 기록과 한계는
[데이터 생성 실측](2026-09-05-diverse-behavior-data.md)에 보존한다.

## 29. 평가 전 학습 입력 조립과 15/10피처 조건 고정 — #109

#107 raw 이력은 아직 평가 slate가 없고 world별 ID prefix도 기존 candidate 로더와
달랐다. 평가 manifest를 가장하지 않고 별도 학습 bundle을 만들었다. 9/2의 30분 클릭
귀속 label과 9/1까지의 이력으로 실제 E5 피처를 조립하고, 동일 source ID 순서의
15/10열 행렬 및 seed 401/402/403의 공통 60/20/20 분할을 고정했다.

3개 bundle은 노출 4,608행·양성163개이며 warm-up 노출145,560개를 fit label에서
제외했다. 47.020초에 조립·재로딩했고 실제 E5 cache hit54/miss0/새inference0였다.
독립 리뷰가 원본 클릭 귀속·최근7일 피처·9분할/IDhash/weight 및 모델 파일을 대조했다.
관련 회귀19개와 독립 리뷰를 통과했으며 fit/평가/final은 실행하지 않았다.

후속 비교는 18모델·36관측, 신규 cohort10901/10902/10903, cohort별 validation800명/
...
## 31. #113 — 다양한 행동 데이터의 실제 15/10피처 평가

**문제:** #109에서 학습 입력을 고정하고 #111에서 신규 평가 cohort를 봉인했지만, 최근 행동 피처가 실제 학습에 쓰이고 ranking 품질을 일관되게 높이는지는 미검증이었다.

**해결:** 18개 모델을 먼저 학습·봉인한 뒤 전체 validation 유효성을 검사하고, 그룹별 final을 한 번만 소비했다. 각 그룹의6개 예측을 모두 봉인한 뒤 채점해 동일 모델을 재사용했으며, 다른 출력/worktree의 재실행도 전역 claim으로 차단했다. 고정된 판정 기준과30분 제한을 유지했다.

**결과:** 실행부155.321초에18fit·36평가·3final소비를 완료했다. Final NDCG@10 평균은10피처0.475404에서15피처0.496877로 상승했지만, 개선은5/9쌍으로 기준6/9에 미달했고 Recall은0.881634→0.852081로 감소했다. Coverage는 유효했으나 고정 판정은 **not_supported**였다. 독립 저장 지표 재집계·모델/예측/원본 hash 검증에 P1/P2는 없었다.

**한계와 후속:** 최근 행동4개 열이 실제 모델 split에 사용됐지만 click/view 두 열은 전행 동일해 view 열은 미사용이었다. 합성 데이터의 중복 피처 및 Recall 감소 원인은 후속 분석 대상으로 남긴다. 결과를 보고 기준을 낮추거나 동일 final을 다시 소비하지 않는다. 실제 사용자 CTR 개선이나 production 승격을 주장하지 않는다.

근거: [실측 보고서](2026-09-05-diverse-behavior-ablation.md), #113.

## 32. #115 — Recall 감소와 NDCG 상승의 순위 기여 분해

**문제:** #113에서 정답 포함률은 낮아졌는데 상위 순위 점수는 높아져, 평균 지표만으로 손익을 설명하기 어려웠다.

**해결:** 저장된 validation 예측 18개와 정답을 키·해시로 대조하고, 클릭 양성 목록 689개×학습 seed 3개의 정답 순위를 비교했다. cohort별 표본 수를 반영한 가중치로 원래 9개 실험 평균을 재현한 뒤 top10 진입·이탈과 내부 순위 변화를 분리했다.

**결과:** 진입 107건보다 이탈 137건이 많아 Recall Δ−0.014261을 설명했다. 내부 순위 변화의 NDCG 순기여 +0.009940이 진입·이탈 순손실 −0.005087을 상쇄해 NDCG Δ+0.004853이 됐다. 16개 후보 목록은 NDCG 양의 기여, 24개 후보 목록은 NDCG·Recall 모두 손실 기여였다. 기존 지표 18개를 1e-12 이내로 재현했고 입력 58파일의 해시를 보존했다.

**한계와 후속:** 이는 validation의 사후 분석이며 특정 피처의 인과 효과를 확정하지 않는다. 사용자 활동 피처가 목록 내 상수라는 점과 binary 학습 목적을 확인했으나, 순위 학습 대조는 아직 수행하지 않았다. 공식 채점·모델 실행·final 재소비 없이 원래 판정을 유지했다. 상세 집계와 재현 방법은 [실측 보고서의 #115 분석](2026-09-05-diverse-behavior-ablation.md)에 기록한다.

## 33. #117 — Recall 원인 가설을 분리하고 개선 후보의 채택을 거절한 실험

**문제:** #115는 top10 이탈·진입으로 Recall 하락을 설명했지만 복잡도, 학습 목표,
영상별 선호정보, 학습 표본의 인과 효과는 미검증이었다. 기존 행 단위 분할을 ranker에
그대로 쓰면 slate가 분리되며, rank 점수를 클릭 확률로 간주하면 확률 지표 비교도 성립하지 않는다.

**해결:** 실험표와72학습시도/7200초/API0 예산을 먼저 등록했다. 사용자 단위 분할,
기본 fit의 vocabulary, 자연 노출 분포의 별도 calibration을 모든 arm에 적용했다.
36개 모델과36개 보정기로 네 가설을 단일요인 비교하고, 개발에서 고른 선호 피처 후보와
모델·전처리·평가기준을 봉인한 뒤 새 사용자 cohort를 생성해 기존 모델로만 final을 평가했다.
raw ranking과 보정 확률 지표를 분리하고 기존 final·소비기록을 보존했다.

**결과:** 복잡도 축소·LambdaRank·60→80% 표본 증가는 개발 Recall을 개선하지 못했다.
영상 카테고리의 과거7/30일 클릭비율을 추가한 후보는 신규 final에서 Recall0.815790→0.973156,
NDCG0.434282→0.616149로 개선했지만 Brier0.032651→0.032752 악화로 **not_supported·채택 거절**이다.
작은 악화라는 이유로 사전 조건을 완화하지 않았다. 총72fit·54평가·3신규final 소비,
감독 프로세스235.290초, 유료API0회다. 독립 저장지표 재집계에서 판정과 hash의 불일치는 없었다.

**한계와 후속:** 카테고리 선호를 정답 생성에 사용하는 합성 분포의 결과다. 특정 개입의
ranking 효과를 #113 하락의 유일한 원인이나 실제 사용자 CTR 개선으로 확대하지 않는다.
Brier 악화의 세부 원인과 확률 보정 안정성은 미확인이다. 추가 연구에는 별도 사전등록과
신규 평가가 필요하며, 이번 작업에서 배포·production 승격은 수행하지 않았다.

근거: [통제 실험 보고서](2026-09-06-recall-controlled-experiment.md), #117.
