...
상태: 구현·실험 완료, `not_supported`. 기준 코드 `45c416a5ad1f4510ea6c690203fb9fd0662dc642`.
...
사전 등록 `44d648a`, 실행 코드 `1a771e8`. 36회 비교를 779.014초에 완료했다.
날짜순 생성·warm-up 학습 제외·당일/미래 행동 제거 무영향을 실제 데이터에서도 확인했다.
최근 시청 시간은 비상수이며 포함 모델 18개 모두 사용했지만, NDCG@10은 validation
0.934282, final 0.945335로 두 arm이 모든 paired 관측에서 동률이었다.
Recall은 모두 1.0이다. 일부 확률 지표의 평균 개선은 있지만 primary 개선이 없어
`not_supported`로 종료했다. 새로운 final 3개만 한 번씩 소비했다.
상세는 [실측 보고서](../../reports/2026-09-05-temporal-warmup-experiment.md)를 참조한다.
